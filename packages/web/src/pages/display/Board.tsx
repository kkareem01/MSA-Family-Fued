...
/** Two columns read top-to-bottom: ranks 1-4 on the left, 5-8 on the right, like the show. */
export function Board({ board }: Props) {
  const count = board?.answers.length ?? 0;
  const visibleSlots = SLOTS.filter((rank) => rank <= Math.max(count, ROWS));
  const answerAt = (rank: number) => board?.answers.find((a) => a.rank === rank) ?? null;
  const left = visibleSlots.filter((rank) => rank <= ROWS);
  const right = visibleSlots.filter((rank) => rank > ROWS);
  return (
